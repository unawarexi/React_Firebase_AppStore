        ...
        <div className="pt-12 pb-4 px-4">
          ...
          <div className="mt-6 grid grid-cols-3 gap-4 text-center">
            <div className="bg-gray-100 rounded-lg p-3">
              <div className="text-2xl font-bold text-gray-900">{user.stats.wins ?? '-'}</div>
              <div className="text-xs text-gray-500">Wins</div>
            </div>
            <div className="bg-gray-100 rounded-lg p-3">
              <div className="text-2xl font-bold text-gray-900">{user.stats.matches ?? '-'}</div>
              <div className="text-xs text-gray-500">Matches</div>
            </div>
            <div className="bg-gray-100 rounded-lg p-3">
              <div className="text-2xl font-bold text-gray-900">{user.stats.kd ?? '-'}</div>
          ...
          <div className="mt-6 border-t border-gray-200 pt-4">
            ...
            <div className="flex flex-wrap gap-2">
              <div className="bg-gray-100 rounded-full px-3 py-1 text-xs flex items-center text-gray-900">
  ...
  return (
    ...
    >
      <motion.div 
        ...
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-xl font-bold mb-4 text-gray-900">Send Friend Request</h2>
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            ...
            <div className="text-sm text-gray-500">{selectedUser?.rank} • Level {selectedUser?.level}</div>
          </div>
        </div>
        <div className="mb-4">
          <label className="block text-gray-600 mb-2 text-sm">Add a message (optional)</label>
          <textarea 
            value={message}
            onChange={(e) => setMessage(e.target.value)}
            className="w-full bg-gray-100 text-gray-900 rounded-lg p-3 focus:ring-2 focus:ring-blue-500 focus:outline-none h-24 text-sm"
  ...
  return (
    ...
    >
      <motion.div 
        ...
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-xl font-bold mb-4 text-gray-900">Invite to Group</h2>
        <div className="flex items-center space-x-3 mb-4">
          <img 
            src={selectedUser?.avatar} 
            alt={selectedUser?.name}
          ...
          <div>
            <div className="font-medium text-gray-900">{selectedUser?.name}</div>
            <div className="text-sm text-gray-500">Currently playing: {selectedUser?.game}</div>
          </div>
        </div>
        <div className="mb-4">
          <label className="block text-gray-600 mb-2 text-sm">Select a group</label>
          <div className="space-y-2">
            {myGroups.map(group => (
              <div 
                key={group.id}
                className={`border rounded-lg p-3 cursor-pointer transition-colors ${
                  selectedGroup === group.id ? 'border-blue-500 bg-blue-100 bg-opacity-40' : 'border-gray-200 hover:border-gray-300'
                }`}
    ...
    <div className="pb-20">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button 
            className={`px-3 py-1 rounded-full text-sm transition-colors ${
              activeFilter === 'online' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'
            }`}
          ...
          <button 
            className={`px-3 py-1 rounded-full text-sm transition-colors ${
              activeFilter === 'friends' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'
            }`}
      ...
      >
        {filteredUsers.map(user => (
          <motion.div 
            key={user.id}
            className="bg-white rounded-xl p-3 flex items-center justify-between border border-gray-100 hover:bg-gray-50 transition-colors"
