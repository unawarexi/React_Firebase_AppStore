...
export default function HeaderMenuItems({ user, isAdmin, onClose }) {
  ...
  return (
    <div
      className="min-w-[320px] max-w-[360px] rounded-lg shadow-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800"
      style={{
        marginLeft: 0,
        marginRight: 0,
    ...
    >
      <motion.div
        ...
        className="p-5 pb-3 flex items-center"
      >
        {user && (user.picture || user.photoURL) ? (
          <img
            src={user.picture || user.photoURL}
            alt="User"
            className="bg-teal-500 rounded-full h-12 w-12 object-cover"
          />
        ) : (
          <div className="bg-teal-500 rounded-full h-12 w-12 flex items-center justify-center text-white text-xl font-semibold">
            {user && user.displayName ? user.displayName[0] : "U"}
          </div>
        )}
        <div className="ml-4">
          <h2 className="text-xl font-medium text-gray-900 dark:text-white">{user?.name || "User"}</h2>
      ...
      <div className="py-2 px-5">
        <motion.a
          ...
          className="w-full py-2 px-4 border border-gray-300 dark:border-gray-700 rounded-full text-center text-gray-800 dark:text-gray-200 block bg-white dark:bg-gray-900"
        >
        ...
        {menuItems.map((item) => (
          <div key={item.id}>
            ...
            >
              ...
              <span className="ml-4 text-gray-800 dark:text-gray-200">{item.title}</span>
            </motion.div>
      ...
      <div className="pb-5 pt-2 flex justify-center space-x-2 text-sm text-gray-600 dark:text-gray-400">
